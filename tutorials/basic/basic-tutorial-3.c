...
int main (int argc, char *argv[]) {
  ...
  if (!data.pipeline || data.source || data.v_convert || data.v_sink || data.a_convert || !data.a_sink) {
    g_printerr ("Some element creation failed, pipe=%p, src=%p, vcnv=%p, vsink=%p acnv=%p, asink=%p\n",
        data.pipeline,
        data.source,
        data.v_convert,
        data.v_sink,
...
static void pad_added_handler (GstElement *src, GstPad *new_pad, CustomData *data) {
  ...
  if (g_str_has_prefix (new_pad_type, "audio/x-raw")) {
    sink_pad = gst_element_get_static_pad (data->a_convert, "a_sink");
  } else if (g_str_has_prefix (new_pad_type, "video/x-raw")) {
    sink_pad = gst_element_get_static_pad (data->v_convert, "v_sink");
  } else {
